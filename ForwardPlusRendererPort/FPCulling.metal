...
static float unproject_depth(constant FPFrameData & frameData, float depth)
{
...
static float2 screen_to_view_at_z1(constant FPFrameData & frameData, ushort2 screen)
{
...
static float distance_point_plane(thread const FPPlane& plane, float3 point)
{
...
kernel void cull_lights(imageblock<ColorData,imageblock_layout_implicit> imageBlock,
                        constant FPFrameData & frameData    [[ buffer(FPBufferIndexFrameData) ]],
                        device vector_float4 *light_positions [[ buffer(FPBufferIndexLightsPosition)]],
                        threadgroup int *visible_lights       [[ threadgroup(FPThreadgroupBufferIndexLightList)]],
                        threadgroup TileData *tile_data       [[ threadgroup(FPThreadgroupBufferIndexTileData)]],
                        ...
                        uint thread_linear_id                 [[ thread_index_in_threadgroup]])
{
    ...
    for (uint baseLightId = 0; baseLightId < FPNumLights; baseLightId += threadgroup_linear_size)
    {

        // Get the light ID for the current thread and iteration.
        uint lightId = baseLightId + thread_linear_id;
        if(lightId > FPNumLights-1)
        {
            break;
        }
