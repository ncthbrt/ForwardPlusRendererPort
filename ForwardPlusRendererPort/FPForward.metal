...
forward_lighting_fragment(ColorInOut               in              [[ stage_in ]],
                          ...
                          device FPPointLight   *light_data      [[ buffer(FPBufferIndexLightsData) ]],
                          device vector_float4    *light_positions [[ buffer(FPBufferIndexLightsPosition) ]],
                          texture2d<half>          baseColorMap    [[ texture(FPTextureIndexBaseColor) ]],
                          texture2d<half>          normalMap       [[ texture(FPTextureIndexNormal) ]],
                          texture2d<half>          specularMap     [[ texture(FPTextureIndexSpecular) ]])
{
    constexpr sampler linearSampler(mip_filter::linear, mag_filter::linear, min_filter::linear, s_address::repeat, t_address::repeat);

    int num_lights = min(atomic_load_explicit(&tile_data->numLights,memory_order_relaxed),FPMaxLightsPerTile);
    ...
    for(int i = 0; i < num_lights; i++)
    {
        ...
        device FPPointLight &light = light_data[lightId];
